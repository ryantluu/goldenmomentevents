...
} = require(`./utils`); // If the image is hosted on contentful
...
module.exports = async ({
  ...
  const defaults = {
    ...
    showCaptions: false,
    pathPrefix // This will only work for markdown syntax image tags

  };
  const markdownImageNodes = select(markdownAST, `image`); // This will also allow the use of html image tags
  ...
  const generateImagesAndUpdateNode = async function (node, resolve) {
    ...
    const response = await axios({
      method: `GET`,
      url: `https:${node.url}`,
      // for some reason there is a './' prefix
      responseType: `stream`
    });
    ...
    const presentationWidth = responsiveSizesResult.presentationWidth; // Generate default alt tag

    const originalImg = node.url;
    const fileNameNoExt = fileName.replace(/\.[^/.]+$/, ``);
    const defaultAlt = fileNameNoExt.replace(/[^A-Z0-9]/gi, ` `); // Construct new image node w/ aspect ratio placeholder
