...
function () {
  var _ref = (0, _asyncToGenerator2.default)(function* ({
    ...
    const online = yield isOnline(); // If the user knows they are offline, serve them cached result
    ...
    const createSyncToken = () => `${options.spaceId}-${options.environment}-${options.host}`;
    ...
    if (store.getState().status.plugins && store.getState().status.plugins[`gatsby-source-contentful`] && store.getState().status.plugins[`gatsby-source-contentful`][createSyncToken()]) {
      syncToken = store.getState().status.plugins[`gatsby-source-contentful`][createSyncToken()];
    }

    const _ref2 = yield fetchData(Object.assign({
      syncToken
    }, options)),
          currentSyncData = _ref2.currentSyncData,
          contentTypeItems = _ref2.contentTypeItems,
    ...
    function deleteContentfulNode(node) {
      const localizedNodes = locales.map(locale => {
        const nodeId = createNodeId(normalize.makeId({
          id: node.sys.id,
    ...
    const existingNodes = getNodes().filter(n => n.internal.owner === `gatsby-source-contentful`);
...
function () {
  var _ref3 = (0, _asyncToGenerator2.default)(function* ({
    ...
    yield fs.copy(require.resolve(`gatsby-source-contentful/src/fragments.js`), `${program.directory}/.cache/fragments/contentful-asset-fragments.js`);
  });
