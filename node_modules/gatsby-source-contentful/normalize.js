    ...
    const entryNodes = entries.map(entryItem => {
      ...
      Object.keys(entryItemFields).forEach(entryItemFieldKey => {
        ...
        if (fieldType === `Text`) {
          ...
          delete entryItemFields[entryItemFieldKey];
        } else if (fieldType === `RichText` && process.env.GATSBY_CONTENTFUL_RICH_TEXT === `enabled` && _.isPlainObject(entryItemFields[entryItemFieldKey])) {
          const richTextNode = prepareStructuredTextNode(entryNode, entryItemFieldKey, entryItemFields[entryItemFieldKey], createNodeId);
